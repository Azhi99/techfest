    ...
    <div>
        <v-progress-linear :indeterminate="true" v-if="loading"></v-progress-linear>
            <v-container v-else class="my-2">
                ...
                <form @submit.prevent="addVisitorCheckInByVisitorCode" autocomplete="off">
                    <v-layout row wrap>
                        <v-flex xs12 lg2 xl2 md3 sm4 class="px-1">
                            <v-text-field autofocus label="Visitor Code" filled dense v-model="visitor_code" required></v-text-field>
                        </v-flex>
                
                        <v-flex xs12 lg2 xl2 md2 sm4 class="mx-1">
                            <v-btn  color="primary" :loading="loading_btn" type="submit" x-large>{{$store.getters.language.data.visitor_check_in.add_btn}}</v-btn>
                        </v-flex>
                    </v-layout>
                </form>
...
<script>
    ...
    export default {
        ...
        methods: {
            addVisitorCheckInByVisitorCode() {
                this.loading_btn = true;
                var d = new Date();
                d.setHours(d.getHours() + 3)
                requests.createVisitorCheckInByVisitorCode({
                    visitor_code: this.visitor_code,
                    visitor_check_in_date_time: d
                }).then((r) => {
                    this.visitor_code = null
