...
class VisitorCheckIn {
    ...
    async report(columns) {
        ...
        if (columns.visitor_id) {
            query += ` AND visitor_id = '${columns.visitor_id}'`
        }
            
        if (columns.visitor_check_in_date_time) {
    ...
    async deleteByID(id) {
        const deletedData = await db(table_name).where('visitor_check_in_id', id).del().then(data => {
            return data
        });
        return deletedData
    }
    ...
    async deleteList(list) {
        const deletedData = await db(table_name).whereIn('visitor_check_in_id', list).del().then(data => {
            return data
        });
        return deletedData
    }
}
module.exports = new VisitorCheckIn()
