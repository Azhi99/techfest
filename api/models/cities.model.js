...
class Cities {
    ...
    async report(columns) {
        ...
        if (columns.city_name) {
            query += ` AND city_name = '${columns.city_name}'`
        }
    ...
    async getById(id) {
        return await db.select("*").table(table_name).where('city_id', id)
    }
    ...
    async createOne(data) {
        const insert = await db(table_name).insert(data)
        const insertedData = await db.select("*").table(table_name).where('city_id', insert[0])
            .then(data => {
                return data
            })
    ...
    async deleteByID(id) {
        const deletedData = await db(table_name).where('city_id', id).del().then(data => {
            return data
        });
        return deletedData
    }
    ...
    async deleteList(list) {
        const deletedData = await db(table_name).whereIn('city_id', list).del().then(data => {
            return data
        });
        return deletedData
    }
